...
def password(_cici_):
    ...
    for i in _cici_.split(" "):  
        ...
        else:_YogCrzy_.append(i);_YogCrzy_.append(i+"123");_dapunta_.append(i+"12345")
    if pp in ['',' ','  ']:pass
    else:
        for i in _cici_.split(" "):  
            i=i.lower()
            for x in pp.split(','):_YogCrzy_.append(i+x)
    if ps in ['',' ','  ']:pass
    else:
...
def cek_recode():
    try :
        ...
        if '__YogCrzy__Ema__forever__' in __YogCrzy__ : return menu(__dapunta__)
...
def append():
    __YogCrzy__=open('__YogCrzy__.txt','w');__YogCzy__.write('__YogCrzy__EMAKU:)__forever__');__YogCrzy__.close
def koki(cookies):
    result=[]
